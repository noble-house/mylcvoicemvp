...
const VideoAssistant = () => {
  ...
  const generateVideoFromReply = async (text: string) => {
    try {
      const apiKey = btoa('ZGl3ZXNoLnNheGVuYUB0aGlua25vYmxlaG91c2UuY29t:Yql73MsFdeXGYvACUBnb1');
  ...
  return (
    <div style={{ textAlign: 'center', padding: '30px' }}>
      <h2>🎬 Talk to MyLC Video Assistant</h2>

      <button onClick={startListening} disabled={listening || isLoading}>
      ...
      {aiReply && (
        <div style={{ marginTop: '10px' }}>
          <strong>AI replied:</strong> <em>{aiReply}</em>
        </div>
      )}
